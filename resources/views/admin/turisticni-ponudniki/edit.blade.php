@php
    ...
    if(isset($_GET['id'])){
        ...
        $email =  $kontakti ? array_values(array_filter($kontakti, function ($el){
            return $el['vrsta'] === 'email';
        })) : [];
        $email = count($email) > 0 ? $email[0]['kontakt'] : '';
        $spletnastran = $kontakti ? array_values(array_filter($kontakti, function ($el){
            return $el['vrsta'] === 'web';
        })) : [];
        $spletnastran = count($spletnastran) > 0 ? $spletnastran[0]['kontakt'] : '';
        $telefoni = $kontakti ? array_values(array_filter($kontakti, function ($el){
            return $el['vrsta'] === 'tel';
        })) : [];
        $telefon = count($telefoni) > 0 && $telefoni[0]['kontakt'];
        $stelefon = count($telefoni) > 1 && $telefoni[1]['kontakt'];
        $social = get_field('druzbena_omrezja', $id);
        $facebook = $social ? array_values(array_filter($social, function ($el){
            return $el['platforma'] === 'facebook';
        })) : [];
        $facebook = count($facebook) > 0 ? $facebook[0]['povezava'] : '';
        $instagram = $social ? array_values(array_filter($social, function ($el){
            return $el['platforma'] === 'instagram';
        })) : [];
        $instagram = count($instagram) > 0 ? $instagram[0]['povezava'] : '';
...
@if($id && $ponudnik)
    <section class="flex flex--center">
        <div class="width100 pt24" style="max-width: 900px">
            <a href="{{admin_url('admin.php?page=turisticni-ponudniki')}}" class="flex flex--middle">@include('icons.chevron-left')
                Vsi turistični ponudniki</a>
            <h3 class="mb8">Uredi {{$ponudnik->post_title}}</h3>
            <div>
                <form class="row" id="formadminturisticniponudnikedit" novalidate>
...
@else
    <section class="pt40">
        <div class="row flex--center">
            <div class="col-md-8">
                <h3 class="text-center mb8">Turistični ponudnik ne obstaja</h3>
                <div class="flex flex--center">
                    <a href="{{admin_url('admin.php?page=turisticni-ponudniki')}}" class="btn" style="color: white !important;">
                        Nazaj na turistične ponudnike
                    </a>
                </div>
            </div>
        </div>
